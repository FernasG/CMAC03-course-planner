...
from grafo_disciplinas_si import grafoSI
...
pesos = gerar_pesos_das_arestas(grafoSI)
G = nx.DiGraph()

for d, dados in grafoSI.items():
...
for node, dados in grafoSI.items():
...
ax = plt.gca()
ax.set_title("Grafo de Disciplinas - Curso SIN", fontsize=20, pad=25)
...
plt.axis("off")
plt.tight_layout()
plt.savefig("grafo_setas_visiveissin.png", dpi=300, bbox_inches='tight')
plt.show()